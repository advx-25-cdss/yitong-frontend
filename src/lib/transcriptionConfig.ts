...
export const MEDICAL_KEYWORDS = {
  doctor: [
    ...
    '症状', '病史', '检验', '化验', '手术', '康复', '复查',
    '用药', '剂量', '疗程', '副作用', '禁忌', '注意事项'
  ],
  patient: [
    '疼痛', '不舒服', '症状', '感觉', '担心', '患者',
    '头痛', '胸闷', '气短', '恶心', '呕吐', '腹痛',
...
export function validateBrowserSupport() {
  const support = {
    ...
    https: typeof window !== 'undefined' && (location.protocol === 'https:' || location.hostname === 'localhost')
  };
...
export function estimateModelLoadTime(modelType: WhisperModelType, connectionSpeed: 'slow' | 'medium' | 'fast' = 'medium') {
  const sizes = {
    tiny: 39,
    small: 244, 
    base: 142,
