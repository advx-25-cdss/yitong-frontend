...
export class TransformersAudioTranscriber {
  ...
  private onTranscriptionUpdate: ((segments: TranscriptionSegment[]) => void) | null = null;
  ...
  private async initializeTranscriber() {
    ...
    try {
      ...
      if (this.config.enableWebGPU && browserSupport.features.webgpu) {
        ...
        console.log('Transformers.js transcriber initialized with WebGPU acceleration');
      } else {
  ...
  setOnTranscriptionUpdate(callback: (segments: TranscriptionSegment[]) => void) {
  ...
  async startTranscription(): Promise<void> {
    ...
    try {
      ...
      this.mediaRecorder.onstop = () => {
        console.log('Recording stopped, audio chunks accumulated for final processing');
      };

      // Start recording with 1-second chunks for smooth accumulation
      this.mediaRecorder.start(1000);
      console.log('Started client-side audio recording (no real-time processing)');
    } catch (error) {
  ...
  stopTranscription(): void {
    ...
    console.log('Stopping client-side transcription and processing complete audio...');
    this.isTranscribing = false;
