...
export const historyPresentIllnessApi = {
  create: (data: Omit<HistoryPresentIllness, '_id' | 'created_at' | 'updated_at'>) =>
    api.post<HistoryPresentIllness>('/api/ehr/history-present-illness/', data),

  getAll: () =>
    api.get<ApiListResponse<HistoryPresentIllness>>('/api/ehr/history-present-illness/'),
  
  getById: (recordId: string) =>
    api.get<HistoryPresentIllness>(`/api/ehr/history-present-illness/${recordId}`),
  
  update: (recordId: string, data: Partial<HistoryPresentIllness>) =>
    api.put<HistoryPresentIllness>(`/api/ehr/history-present-illness/${recordId}`, data),
  
  delete: (recordId: string) =>
    api.delete<never>(`/api/ehr/history-present-illness/${recordId}`),
  
  getByPatient: (patientId: string) =>
    api.get<Array<HistoryPresentIllness>>(`/api/ehr/history-present-illness/patient/${patientId}`),
};

// Past Medical History API
export const pastMedicalHistoryApi = {
  create: (data: Omit<PastMedicalHistory, '_id' | 'created_at' | 'updated_at'>) =>
    api.post<PastMedicalHistory>('/api/ehr/past-medical-history/', data),
  
  getAll: () =>
    api.get<ApiListResponse<PastMedicalHistory>>('/api/ehr/past-medical-history/'),
  
  getById: (recordId: string) =>
    api.get<PastMedicalHistory>(`/api/ehr/past-medical-history/${recordId}`),
  
  update: (recordId: string, data: Partial<PastMedicalHistory>) =>
    api.put<PastMedicalHistory>(`/api/ehr/past-medical-history/${recordId}`, data),
  
  delete: (recordId: string) =>
    api.delete<never>(`/api/ehr/past-medical-history/${recordId}`),
  
  getByPatient: (patientId: string) =>
    api.get<Array<PastMedicalHistory>>(`/api/ehr/past-medical-history/patient/${patientId}`),
};

// Medication History API
export const medicationHistoryApi = {
  create: (data: Omit<MedicationHistory, '_id' | 'created_at' | 'updated_at'>) =>
  ...
  getByPatient: (patientId: string) =>
    api.get<Array<MedicationHistory>>(`/api/ehr/medication-history/patient/${patientId}`),
};
...
export const allergyHistoryApi = {
  ...
  getByPatient: (patientId: string) =>
    api.get<Array<AllergyHistory>>(`/api/ehr/allergy-history/patient/${patientId}`),
};
...
export const vitalSignsApi = {
  ...
  getAll: () =>
    api.get<ApiListResponse<VitalSigns>>('/api/ehr/vital-signs/'),
...
export const casesApi = {
  ...
  getAll: () =>
    api.get<ApiListResponse<Case>>('/api/diagnosis/cases/'),
  
  getById: (caseId: string) =>
    api.get<Case>(`/api/diagnosis/cases/${caseId}`),
  ...
  search: (query: string) =>
    api.get<Array<Case>>(`/api/diagnosis/cases/search/`, { params: { q: query } }),
};
...
export const testsApi = {
  ...
  getAll: () =>
    api.get<ApiListResponse<Test>>('/api/diagnosis/tests/'),
  
  getById: (testId: string) =>
    api.get<Test>(`/api/diagnosis/tests/${testId}`),
  ...
  downloadResults: (testId: string) =>
    api.get(`/api/diagnosis/tests/${testId}/download-results`, { responseType: 'blob' }),
...
export const medicinesApi = {
  ...
  getAll: () =>
    api.get<ApiListResponse<Medicine>>('/api/diagnosis/medicines/'),
  ...
  checkInteractions: (medicineIds: string[]) =>
    ...
    }>(`/api/diagnosis/medicines/interactions/check`, { params: { medicines: medicineIds.join(',') } }),
  
  search: (query: string) =>
    api.get<Array<Medicine>>(`/api/diagnosis/medicines/search/`, { params: { q: query } }),
};
...
export const treatmentsApi = {
  ...
  updateOutcome: (treatmentId: string, outcome: string) =>
    api.patch<Treatment>(`/api/diagnosis/treatments/${treatmentId}/outcome`, { outcome }),
  
  search: (query: string) =>
    api.get<Array<Treatment>>(`/api/diagnosis/treatments/search/`, { params: { q: query } }),
};
...
export const diagnosesApi = {
  ...
  updateStatus: (diagnosisId: string, status: Diagnosis['status']) =>
    api.patch<Diagnosis>(`/api/diagnosis/diagnoses/${diagnosisId}/status`, { status }),
  
  updateFollowUp: (diagnosisId: string, followUp: string) =>
    api.patch<Diagnosis>(`/api/diagnosis/diagnoses/${diagnosisId}/follow-up`, { follow_up: followUp }),
  
  search: (query: string) =>
    api.get<Array<Diagnosis>>(`/api/diagnosis/diagnoses/search/`, { params: { q: query } }),
};
...
export const patientApi = {
  getPatientById: async (patientId: string): Promise<Patient> => {
    try {
      ...
      const pmh = pmhResponse.status === 'fulfilled' ? pmhResponse.value.data[0] : undefined;
      const medicationHistory = medicationHistoryResponse.status === 'fulfilled' ? medicationHistoryResponse.value.data[0] : undefined;
      const allergyHistory = allergyHistoryResponse.status === 'fulfilled' ? allergyHistoryResponse.value.data[0] : undefined;
      const cases = casesResponse.status === 'fulfilled' ? casesResponse.value.data.data : [];
      const tests = testsResponse.status === 'fulfilled' ? testsResponse.value.data.data : [];
      ...
      for (const demo of allDemographics) {
        if (demo.patient_id) {
          try {
            ...
            console.warn(`Failed to fetch data for patient ${demo.patient_id}:`, error);
